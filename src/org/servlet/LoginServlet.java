...
public class LoginServlet extends HttpServlet{
	 ...
	 * 
	 */
	private static final long serialVersionUID = 6332357404492830854L;
	public void doGet(HttpServletRequest request,HttpServletResponse response) throws ServletException,IOException{
		...
		Login l=(Login) session.getAttribute("admin"); // 二次甚至是第三次，就不应该再判断该用户的信息
		
		if(l==null)
			l = loginDao.checkLogin(request.getParameter("name"),request.getParameter("password"));
		if(l!=null){                                      //如果登陆成功
			session.setAttribute("admin",l);              //将获取的对象保存在session中
